...
// HARFANG/macOS-arm64 patch: upstream bgfx (still as of 2025) sets this to
// 8 MB and `setShaderUniform` performs no bounds check, so heavy multi-draw
// scenes (e.g. tutorials/physics_pool_of_objects.lua with several hundred
// dynamic nodes) silently overflow the per-frame uniform ring and produce
// geometry corruption (degenerate triangles, 1-px "needle" artifacts).
// Bumped to 32 MB locally to push the threshold well past any sensible scene
// budget. See specifications/SPECS_INTERACTIVE_BOOK_MACOS_ARM_FEASIBILITY_EN.md.
#define UNIFORM_BUFFER_SIZE (32*1024*1024)
