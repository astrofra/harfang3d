...
-- cube
cube_size =  hg.Vec3(4, 0.5, 1)
cube_ref = res:AddModel('cube', hg.CreateCubeModel(vtx_layout, cube_size.x, cube_size.y, cube_size.z))

-- setup the scene
scene = hg.Scene()

cam_mat = hg.TransformationMat4(hg.Vec3(-5, 2.0, -5), hg.Vec3(hg.Deg(15), hg.Deg(45), 0))
...
cube_node, cube_rb = CreatePhysicCubeEx(scene, cube_size, hg.TranslationMat4(hg.Vec3(0, 0, 0)), cube_ref, {mat_grey}, hg.RBT_Dynamic, 1.0)
cube_rb:SetAngularDamping(0.0)
