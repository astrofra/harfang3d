...
local res_x = 1280;
local res_y = 720;
local win = hg.RenderInit("Harfang - Physics Matrix Interaction", res_x, res_y, hg.RF_VSync | hg.RF_MSAA4X);
...
local ground_mdl = hg.CreateCubeModel(vtx_layout, 100, 0.02, 100);
...
local mat = hg.CreateMaterial(prg_ref, "uDiffuseColor", hg.Vec4(0.5, 0.5, 0.5), "uSpecularColor", hg.Vec4(1, 1, 1));

local scene = hg.Scene();

local cam_mat = hg.TransformationMat4(hg.Vec3(0, 1.5, -5), hg.Deg3(5, 0, 0));
local cam = hg.CreateCamera(scene, cam_mat, 0.01, 1000);
scene.SetCurrentCamera(cam);
local view_matrix = hg.InverseFast(cam_mat);
local c = cam.GetCamera();
local projection_matrix = hg.ComputePerspectiveProjectionMatrix(c.GetZNear(), c.GetZFar(), hg.FovToZoomFactor(c.GetFov()), hg.Vec2(res_x.tofloat() / res_y.tofloat(), 1));

local lgt = hg.CreatePointLight(scene, hg.TranslationMat4(hg.Vec3(3, 4, -6)), 0);

local cube_node = hg.CreatePhysicCube(scene, hg.Vec3(1, 1, 1), hg.TranslationMat4(hg.Vec3(1.25, 2.5, 0)), cube_ref, [mat], 2);
...
while (!keyboard.Pressed(hg.K_Escape) && hg.IsWindowOpen(win)) {
	...
	local begin_result = hg.ImGuiBegin("Transform and Physics", true, hg.ImGuiWindowFlags_AlwaysAutoResize);
	if (begin_result[0]) {
		...
		local pos_result = hg.ImGuiInputVec3("Transform.pos", cube_node.GetTransform().GetPos(), 2);
		if (pos_result[0]) {
			cube_node.GetTransform().SetPos(pos_result[1]);
		}
		...
		local readonly_world_result = hg.ImGuiInputVec3("Transform.GetWorld().T", hg.GetT(cube_node.GetTransform().GetWorld()), 2, hg.ImGuiInputTextFlags_ReadOnly);
